...
public class Assorted {
    ...
    public static List<Integer> sortOdd(List<Integer> list) {
        ...
        List<Integer> oddDex = new ArrayList<>();
        ...
        int h = 0;
        for (int i = 0; i < list.size(); i++) {
            if (oddDex.contains(i)) {
                list.set(i, odd.get(h));
                h++;
            }
        }
    ...
    public static List<Integer> uniqueNumber(int lowerBound, int upperBound) {
        ...
        for (int i = lowerBound; i <= upperBound; i++) {
            ...
            for (int j = 0; j < val.length(); j++) {
                int dig = Character.getNumericValue(val.charAt(j));
                int numToAdd = (int) Math.pow(dig, j + 1);
                sum += numToAdd;
            }
    ...
    public static List<String> wildWest(List<String> directions) {
        ...
        while (a) {
            ...
            for (int i = 0; i < directions.size() - 1; i++) {
                if (areOpposites(directions.get(i), directions.get(i + 1))) {
                    directions.remove(i);
                    directions.remove(i);
                    b = true;
                    break;
    ...
    public static boolean areOpposites(String stringOne, String stringTwo) {
        if (stringOne.equals("NORTH") && stringTwo.equals("SOUTH") || stringOne.equals("SOUTH") && stringTwo.equals("NORTH")) {
            return true;
        }
        if (stringOne.equals("EAST") && stringTwo.equals("WEST") || stringOne.equals("WEST") && stringTwo.equals("EAST")) {
            return true;
        }
        return false;
    }
    ...
    private static void sortTills(List<Integer> tills) {
        for (int i = 0; i < tills.size() - 1; i++) {
            for (int j = i + 1; j < tills.size(); j++) {
                if (tills.get(i) > tills.get(j)) {
                    int temp = tills.get(i);
                    tills.set(i, tills.get(j));
                    tills.set(j, temp);
                }
            }
        }
